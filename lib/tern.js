...
(function(root, mod) {
  ...
  function updateText(file, text, srv) {
    if (typeof text === 'object') {
      var model = text;
      text = model.text;
      model.text = file.text = srv.options.stripCRs ? text.replace(/\r\n/g, "\n") : text;
      file.model = model;
  ...
  Server.prototype = signal.mixin({
    ...
    checkUpdates: function() {
      if (this.fileServer) {
        for (var i = 0; i < this.files.length; i++) {
          var file = this.files[i];
          var filemodel = this.fileServer.getUpdatedFile(this.projectDir + file.name, file.scopeVersion);
          if (filemodel) {
            ensureFile(this, file.name, file.parent, filemodel);
          }
        }
      }
    },
  ...
  function ensureFile(srv, name, parent, text) {
    name = srv.normalizeFilename(name)
  ...
  function analyzeAll(srv, timeBudget, c) {
    ...
    for (var i = 0; i < srv.files.length;) {
      ...
      for (; i < srv.files.length; ++i) {
        ...
        if (file.text == null && file.model == null) done = false;
